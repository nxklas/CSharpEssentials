...
namespace CSharpEssentials.Helpers
{
    ...
    {
        /// <summary>
        /// Creates a <see cref="KeyValuePair{TKey, TValue}"/>[] instance with the content in <paramref name="enumerative"/>
        /// </summary>
        ...
        {
            KeyValuePair<TKey, TValue>[] keyValuePairs = new KeyValuePair<TKey, TValue>[enumerative.Count()];
            int i = 0;

            foreach (KeyValuePair<TKey, TValue> current in enumerative)
            {
                i++;
